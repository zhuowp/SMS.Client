...
namespace SMS.Client.Common.Utilities.Replays
{
    public class VideoReplayHelper : IReplayHelper
    {
        ...
        private bool ValidateReplayParam(ReplayModel replayModel)
        {
            ...
            {
                LogHelper.Default.DebugFormatted("录像播放参数错误，设备信息为空");
                return false;
            }

            if (replayModel.ScreenHandle == IntPtr.Zero)
            {
                LogHelper.Default.DebugFormatted("录像播放参数错误，没有播放控件句柄，通道Id：{0}，IP地址：{1}",
        ...
        private bool ValidateSettingReplaySpeed(double replaySpeed)
        {
            ...
            {
                LogHelper.Default.DebugFormatted("录像回放快放失败，设置的速度大于系统所允许的最大速度");
                return false;
            }

            if (replaySpeed < _minReplaySpeed)
            {
                LogHelper.Default.DebugFormatted("录像回放慢放失败，设置的速度小于系统所允许的最小速度");
                return false;
            }
